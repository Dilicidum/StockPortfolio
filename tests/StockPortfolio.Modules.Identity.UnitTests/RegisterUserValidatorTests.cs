...
{
    private static readonly string ValidPassword = new('a', RegisterUserValidator.MinimumPasswordLength);
    ...
    {
        var result = _validator.Validate(new RegisterUser("not-an-email", ValidPassword));
    ...
    public void Validate_MalformedEmail_NamesTheEmailField()
    {
        ...
        failure.PropertyName.ShouldBe(nameof(RegisterUser.Email));
    ...
    {
        var tooShort = new string('a', RegisterUserValidator.MinimumPasswordLength - 1);

        var result = _validator.Validate(new RegisterUser("ada@example.com", tooShort));

        result.IsValid.ShouldBeFalse();
        var failure = result.Errors.ShouldHaveSingleItem();
        failure.PropertyName.ShouldBe(nameof(RegisterUser.Password));
        failure.ErrorMessage.ShouldContain(
            RegisterUserValidator.MinimumPasswordLength.ToString(System.Globalization.CultureInfo.InvariantCulture));
    }

    [Fact]
    public void Validate_WellFormedEmailAndLongEnoughPassword_Succeeds()
    {
        var result = _validator.Validate(new RegisterUser("ada@example.com", "correct horse battery staple"));
    ...
    public void Validate_PassphraseWithNoDigitsOrSymbols_Succeeds()
    {
        ...
        var result = _validator.Validate(new RegisterUser("ada@example.com", "sixteen lower case"));
    ...
    public void Validate_PasswordEqualToEmail_FailsNamingThePasswordField()
    {
        ...
        var result = _validator.Validate(new RegisterUser(Email, Email));

        result.IsValid.ShouldBeFalse();
        result.Errors.ShouldContain(failure => failure.PropertyName == nameof(RegisterUser.Password));
    }
    ...
    public void Validate_EmptyPassword_ReportsOneMessageNotTwo()
    {
        ...
        var result = _validator.Validate(new RegisterUser("ada@example.com", ""));

        result.Errors.ShouldHaveSingleItem().PropertyName.ShouldBe(nameof(RegisterUser.Password));
    }
}
