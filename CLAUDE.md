...
**Phases 1–3 are functionally complete.** **25 projects**, `dotnet build` clean at 0 warnings, and `docker compose up` brings the whole stack up from a clean volume. Phase 3 shipped MarketData end to end — the dashboard, its P&L arithmetic, the last-known-price fallback and the symbol-existence check. **Phase 3 is deployed and verified on the public URL** (2026-08-05, PR #2). `/api/marketdata/health` returns `{"provider":"Finnhub"}` with a real key, the GitHub Pages SPA renders live market prices against the ACA API, and `POST /api/holdings` with a non-existent symbol returns `UnknownTicker` — the one exit item that could never be tested locally, because `FakeQuoteProvider` accepts any well-shaped ticker by design. Outstanding: `TokenPolicy` values are still provisional.

⚠️ **This paragraph said "nothing is deployed" for three days, and it was false the whole time.** There had been a live, healthy deployment since 2026-08-02 — the Deployment section below named its resource group, so the file contradicted itself. The contradiction survived because the record of that deploy lived in two documents **nothing linked to**: [docs/DEPLOYING.md](docs/DEPLOYING.md) was committed on the `phase-2` branch and never merged, and the [design record](docs/superpowers/specs/2026-08-02-azure-deployment-design.md)'s only inbound reference was inside itself. Both are now linked from here and from Deployment below. Operational documents strand more easily than code — nothing fails to compile when they go missing, and the cost here was an agent confidently reporting a blocker that did not exist. This paragraph used to quote a test count as well; it no longer does, because three copies of one number drifted apart. **Counts live in Tests below and nowhere else.**
...
`FINNHUB_API_KEY` is **set** as of 2026-08-05, so the deployed app serves genuine prices. It is optional and defaults to empty; empty means the public URL prices real tickers from the generated walk, which reads as broken rather than as a thoughtful fallback. The empty path is what makes `docker compose up` work from a clean clone with no registration, and that is the only place it belongs.
...
Cost is bounded by **time, not by budget**. Pay-as-you-go has no Azure spending limit, and a budget only emails — it cannot stop anything. `deploy.yml` stamps a `deleteAfter` tag on the resource group and `teardown.yml` deletes the group once that date passes. **Live deployment, running Phase 3 as of 2026-08-05**: resource group `stockportfolio-rg` in `polandcentral`, ~$1.26/day, API at `stockp-api-qdgz3wugqbihs.icysea-481b5825.polandcentral.azurecontainerapps.io`, SPA at `dilicidum.github.io/StockPortfolio`. Postgres Flexible B1ms and Azure Managed Redis Balanced B0 with HA off — **not** Azure Cache for Redis, which is retiring.
