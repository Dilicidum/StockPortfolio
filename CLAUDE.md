...
| `Modules.MarketData.UnitTests` | 140 | 0 |
| `Modules.Alerts.UnitTests` | 114 | 0 |
| `Architecture.Tests` | 55 | **2** |
| `Api.IntegrationTests` | 182 | 0 |

The browser tests are counted separately by `npm --prefix src/Web test` — **49 passing across 9 files**. These are the only test counts in the repository; do not copy them into another document.
...
- **`Maximum Pool Size=2` on every connection string.** Azure Postgres B1ms allows 35 user connections, and a different username is a different Npgsql pool. **Count what opens a pool, not what is defined.** `db/init/01-roles.sql` creates five roles (`migrator` plus four `*_svc`) and four schemas, but `Program.cs` registers exactly **three** `DbContext`s — Identity's, Portfolio's and Alerts' — so **three Npgsql pools exist per replica**. At `Maximum Pool Size=2` and `maxReplicas: 2` that is 3 × 2 × 2 = **12** of 35, and `migrator` runs as a separate job rather than alongside the API. The default of 100 would make it 600. PgBouncer is unavailable on Burstable. MarketData has no `DbContext` and opens no pool. **Count `AddDbContext` calls; this figure has been published wrong before.**
