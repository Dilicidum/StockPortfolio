...
**Every context retries a failed connection up to three times, waiting at most two seconds between attempts, and that constrains any transaction added later.** `EnableRetryOnFailure(maxRetryCount: 3, maxRetryDelay: TimeSpan.FromSeconds(2))` is on all four. **Two seconds is a ceiling, not the interval** — EF backs off and adds a random jitter, so each wait is somewhere between nothing and two seconds, and three retries take under six seconds in total rather than exactly six. Two consequences, and the second is a standing rule rather than a note. It makes EF buffer every result set, which is a real cost paid on every query. And **an explicit transaction must be opened inside the execution strategy, and the work inside it must be safe to run twice** — EF refuses a user-initiated transaction otherwise, at run time, with a message telling you to use `CreateExecutionStrategy`. There is no explicit transaction in the codebase today; the first one has to obey this. The numbers are deliberate rather than the defaults: six attempts backing off to thirty seconds would leave a request against a stopped Postgres hanging for about a minute, with the browser on a spinner and the readiness probe timing out long before the answer arrived.
...
- **The API must never call `Database.Migrate()` at startup.** Migrations are a separate Container Apps job with `parallelism: 1` and no retries, started by the deploy workflow and waited on before the new image goes live. Two replicas racing the same migration corrupt `__EFMigrationsHistory`, and because each module's history table sits in its own schema the corruption is per-module and reads as several unrelated failures.
...
- **`MarketData:Polling:RetentionMinutes` must stay above `Alerts:MaxWindowMinutes`, and the host refuses to start otherwise.** A window longer than the history kept stops alerts firing with nothing anywhere reporting a fault, so the check is deliberately fatal — which also means a bad pair in `docker-compose.yml` or in the container app's environment takes down `docker compose up`, the P0 gate. The shipped pair is 75 and 60, set in three places: `appsettings.json`, `docker-compose.yml` and `containerapp-api.bicep`.
...
- **Azure Managed Redis is `Microsoft.Cache/redisEnterprise`, and four of its properties have no sensible default.** It is TLS-only on port **10000**, not 6379. It supports exactly one database and that database must be named `default`. `encryption: {}` has to be present even for Microsoft-managed keys. `capacity` applies only to the Enterprise SKUs, so a Balanced SKU must omit it. And `clusteringPolicy` must be **`EnterpriseCluster`** — `OSSCluster` exposes real hash slots, and any multi-key command spanning slots then fails with `CROSSSLOT`.
- **A container registry must pin `roleAssignmentMode: 'LegacyRegistryPermissions'`.** Microsoft is making `AbacRepositoryPermissions` the default for new registries, and the legacy roles — `AcrPull`, `AcrPush`, `AcrDelete` — are not honoured on an ABAC registry at all, so the image pull fails while the role assignment still reads as correct. If it is ever flipped to ABAC, the `AcrPull` assignment must become 'Container Registry Repository Reader' plus 'Container Registry Repository Catalog Lister' in the same commit.
- **A role assignment needs its own Bicep module, or ARM runs it in parallel with the thing that depends on it.** Nothing in the container app references the `AcrPull` assignment, so inside one deployment ARM schedules both at once, the pull loses the race, and the deploy reports an image-pull failure that goes away on a retry. A module gives the container app and the migration job something concrete to `dependsOn`, and module completion is a real ordering barrier — do not inline it back into `main.bicep`. The assignment also needs `principalType: 'ServicePrincipal'`, without which ARM can fail while a freshly created managed identity has not yet replicated into Entra.
...
- **The `aspnet:10.0` runtime image ships neither curl nor wget**, which is why the API had no compose healthcheck for four phases. It does ship bash, and bash's `/dev/tcp` opens the socket, so a real HTTP GET needs no extra package and no image growth: `exec 3<>/dev/tcp/127.0.0.1/8080`, then `printf` the request into it. The `\r\n` must survive YAML as backslash-r backslash-n and be turned into CR LF by `printf`, not by YAML.
...
⚠️ **`teardown.yml` deletes the whole resource group once the `deleteAfter` tag passes**, and a redeploy re-stamps it to today + 14 days. The last deploy was 2026-08-06, so the tag reads **2026-08-20**. Deploying extends the window by using it; not deploying lets it expire. An unreadable or missing tag also deletes — deliberate, since a group with no readable deadline is a group nothing is bounding.
