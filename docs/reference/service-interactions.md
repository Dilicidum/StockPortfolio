...
flowchart TB
    ...
    MD -->|"the last price it saw, so the dashboard survives an outage"| RD
...
| **MarketData** | **nothing** | the last price seen for each ticker | the price provider, over HTTP |
| **Alerts** *(designed)* | its own schema — thresholds and fired alerts | cooldowns, stream tickets, and the channel that pushes an alert to the browser | — |
...
## What Alerts adds when it is built

A background loop that asks the provider for prices on a timer, but only for tickers somebody has an
alert on. It writes a short price history to Redis, compares each new price against it, and pushes a
message to the browser over a long-lived connection when a threshold is crossed. Nothing polls while
nobody has an alert configured.
